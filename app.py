...
def show_admin_settings():
    ...
    with tab1:
        ...
        st.info("회원들이 '관리자 API 사용'을 선택했을 때 사용되는 키입니다.")
        ...
        default_limit = st.number_input("신규 회원 기본 API 사용 한도", min_value=10, max_value=10000, value=int(current_limit), step=10)
    ...
    with tab3:
        ...
        st.warning("카카오 비즈니스 채널 가입 후 설정 가능합니다.")
...
def show_service_management():
    ...
    with tab2:
        st.markdown("### ➕ 새 서비스 추가")
        new_service_name = st.text_input("서비스 이름", placeholder="예: 궁합, 운세 등")
        new_service_desc = st.text_area("서비스 설명", placeholder="서비스에 대한 간단한 설명")
...
def show_content_management():
    ...
    selected_service = st.selectbox("서비스 선택", service_names)
    
    selected_service_id = None
    for s in services:
        if s['name'] == selected_service:
    ...
    with tab1:
        st.markdown(f"### 📑 {selected_service} - 목차 설정")
        st.info("🚧 3단계에서 구현 예정")
    
    with tab2:
        st.markdown(f"### 📜 {selected_service} - 지침 설정")
        st.info("🚧 3단계에서 구현 예정")
    
    with tab3:
        st.markdown(f"### 🖼️ {selected_service} - 속지 설정")
        st.info("🚧 3단계에서 구현 예정")
